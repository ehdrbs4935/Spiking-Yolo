...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (SDetect, Detect, Segment)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (SDetect, Detect, Segment, Pose)):
            s = 256  # 2x min stride
    ...
    def _predict_augment(self, x):
        """Perform augmentations on input image x and return augmented inference and backbone_C2f_Bottleneck outputs."""
        ...
        return torch.cat(y, -1), None  # augmented inference, backbone_C2f_Bottleneck
...
class Ensemble(nn.ModuleList):
    ...
    def forward(self, x, augment=False, profile=False, visualize=False):
        ...
        return y, None  # inference, backbone_C2f_Bottleneck output
        ...
        if e.name == 'models':
            raise TypeError(
                emojis(f'ERROR ❌️ {weight} appears to be an Ultralytics YOLOv5 model originally trained '
                       ...
                       f"\nRecommend fixes are to backbone_C2f_Bottleneck a new model using the latest 'ultralytics' package or to "
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, SDetect, Segment):
            m.inplace = inplace
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        elif m in (SDetect, Detect, Segment, Pose):
